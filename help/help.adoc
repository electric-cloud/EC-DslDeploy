...
|Pool or Resource | Required. The name of the pool or resource where the artifact will be retrieved. The DSL is evaluated on the server so all system actions, such as file reads, will be evaluated on a server node.

Default value is `local`.
|Retrieve on All server nodes | Required. Use this option when running in a cluster with a local workspace, as the artifact version needs to be retrieved on all nodes (as the system pick a random server node for each action). Use alongside a resource pool covering all server node agents for the parameter above.
...
Import DSL file structure from a Git repository. This procedure main usage is to set up a CI process.
...
|Exclude objects |New line separated list of paths to objects to exclude. Exclude nothing by default.
