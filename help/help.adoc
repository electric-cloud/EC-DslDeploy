...
.GenerateDslToDirectory output example
[%collapsible]
====
In the following *GenerateDslToDirectory* output example, you can see the directory structure generated for two projects, `testProject1` (line 3) and `testProject2` (line 58), and two resources, `testResource1` (line 114) and `testResource2` (line 127) into a `/tmp/dsl` directory.

[source,shell,linenums,role="novalidate"]
