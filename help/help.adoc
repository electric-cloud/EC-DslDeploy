...
[%collapsible]
====
In the following *InstallDslFromDirectory* input example, `/projects` is being used to install the projects `FOO`, `BAR`, and `PIPE``.
...
[%collapsible]
====
In the following *InstallProject* input example, you can see a sample directory structure to install a project. In this example, `FOO` project with the `BAR` procedure.
